...
const SB = {
  ...
  async loadDocs() {
    ...
    // Replace DOCS entirely from Supabase (authoritative source)
    // This avoids duplicates from local+remote merge
    if (typeof DOCS !== 'undefined' && Array.isArray(DOCS)) {
      DOCS.length = 0;
      fromSupa.forEach(d => DOCS.push(d));
      const _orig = localStorage.setItem.bind(localStorage);
...
function ofRenderNode(node) {
  ...
  el.innerHTML = `
    <div class="of-node-header" data-node="${node.id}">
      ...
      ${['model', 'image-gen', 'video-gen'].includes(node.type) ? `<div class="of-node-expand" onclick="ofToggleExpand('${node.id}')" title="Expandir/Colapsar">⊞</div>` : ''}
...
function ofRenderConnections() {
  ...
  // Use the canvas-wrap (the unscaled container) as the reference rect
  // so that pan/zoom transforms don't skew the connection endpoints.
  const wrap = document.getElementById('of-canvas-wrap');
  if (!wrap) return;
  ...
  OF.connections.forEach((conn, i) => {
    ...
    // Convert screen coords → canvas-local coords (accounting for pan+zoom)
    const x1 = (fp.left + fp.width / 2 - wRect.left - OF.offsetX) / OF.scale;
    const y1 = (fp.top + fp.height / 2 - wRect.top - OF.offsetY) / OF.scale;
    const x2 = (tp.left + tp.width / 2 - wRect.left - OF.offsetX) / OF.scale;
    const y2 = (tp.top + tp.height / 2 - wRect.top - OF.offsetY) / OF.scale;
    const cx = (x1 + x2) / 2;
...
function ofToggleExpand(nodeId) {
  const el = document.getElementById('of-node-' + nodeId);
  if (!el) return;
  const body = el.querySelector('.of-node-body');
  const btn = el.querySelector('.of-node-expand');
  const expanded = el.classList.toggle('of-node-expanded');
  if (btn) btn.textContent = expanded ? '⊟' : '⊞';
  if (body) {
    body.style.maxHeight = expanded ? '' : null;
    body.style.overflow = expanded ? 'visible' : '';
  }
  // Redraw connection lines since node size changed
  ofRenderConnections();
}
