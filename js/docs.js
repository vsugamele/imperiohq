...
function closeDocModal() {
  ...
  docEditId = null;
  // Reset preview state
  const prev = document.getElementById('doc-md-preview');
...
// ── 4.1 Markdown Preview ─────────────────────────────────────────
function docMdToHtml(md) {
  if (!md) return '';
  let h = md
    .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    // fenced code blocks
    .replace(/```([\s\S]*?)```/g, '<pre style="background:var(--surface2);border-radius:6px;padding:10px;margin:8px 0;overflow-x:auto;font-size:11px"><code>$1</code></pre>')
    // inline code
    .replace(/`([^`]+)`/g, '<code style="background:var(--surface2);padding:2px 5px;border-radius:3px;font-size:11px">$1</code>')
    // headings
    .replace(/^### (.+)$/gm, '<h3 style="font-size:13px;margin:12px 0 4px;color:var(--text)">$1</h3>')
    .replace(/^## (.+)$/gm, '<h2 style="font-size:15px;margin:14px 0 5px;color:var(--text)">$1</h2>')
    .replace(/^# (.+)$/gm, '<h1 style="font-size:17px;margin:14px 0 6px;color:var(--gold)">$1</h1>')
    // bold / italic
    .replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
    .replace(/\*(.+?)\*/g, '<em>$1</em>')
    // blockquotes
    .replace(/^&gt; (.+)$/gm, '<blockquote style="border-left:3px solid var(--gold);margin:6px 0;padding:4px 12px;color:var(--text3)">$1</blockquote>')
    // unordered list items
    .replace(/^[\-\*] (.+)$/gm, '<li style="margin:3px 0;color:var(--text2)">$1</li>')
    // ordered list items
    .replace(/^\d+\. (.+)$/gm, '<li style="margin:3px 0;color:var(--text2)">$1</li>')
    // horizontal rule
    ...
    // paragraphs
    .replace(/\n{2,}/g, '</p><p style="margin:6px 0;color:var(--text2)">')
    .replace(/\n/g, '<br>');
  return '<div style="font-size:12px;line-height:1.7"><p style="margin:6px 0;color:var(--text2)">' + h + '</p></div>';
}

function docToggleMdPreview() {
  const btn = document.getElementById('doc-preview-btn');
  const prev = document.getElementById('doc-md-preview');
  const ta = document.getElementById('doc-f-body');
  if (!prev || !ta) return;
  const visible = prev.style.display !== 'none';
  if (visible) {
    ...
    ta.style.width = '50%';
    prev.innerHTML = docMdToHtml(ta.value);
    ta.oninput = () => { prev.innerHTML = docMdToHtml(ta.value); };
...
function docUpdatePreview() {
  const prev = document.getElementById('doc-md-preview');
  const ta = document.getElementById('doc-f-body');
  if (prev && ta && prev.style.display !== 'none') {
    prev.innerHTML = docMdToHtml(ta.value);
  }
}
